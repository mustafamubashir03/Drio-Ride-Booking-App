...
export default function AuthLayout({ children, title, subtitle, badge }: AuthLayoutProps) {
  return (
    <div className="flex min-h-dvh flex-col bg-background pt-[env(safe-area-inset-top)] lg:flex-row lg:pt-0">
      {/* ── Left branding panel ─────────────────────────────────── */}
      <div
        className="relative h-[clamp(7rem,16svh,10rem)] w-full shrink-0 overflow-hidden rounded-t-[1.75rem] bg-drio-deep lg:flex lg:h-auto lg:w-[44%] lg:flex-col lg:rounded-none"
      >
        ...
        {/* Mobile: branding only. The page heading and its subtitle belong to the
            form below, so rendering them here too would repeat them directly above
            the card and cost the vertical room the form needs on a phone. */}
        <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-5 text-center lg:hidden">
          <Logo className="mx-auto !text-[2.1rem]" />
          {badge && (
      ...
      {/* ── Right form panel ───────────────────────────────────── */}
      <div className="relative flex w-full grow items-start justify-center px-6 pb-4 pt-6 sm:items-center sm:px-8 sm:py-12 lg:w-[56%] lg:grow-0">
        {/* Mobile glows */}
        ...
        <div className="relative -mt-7 w-full max-w-[420px] rounded-t-[1.75rem] border-x border-t border-border bg-background/95 px-5 pb-7 pt-6 text-center shadow-2xl backdrop-blur-md [&_label]:text-left sm:mx-4 sm:max-w-[440px] sm:rounded-[1.75rem] sm:border lg:mx-0 lg:mt-0 lg:max-w-[420px] lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:text-left lg:shadow-none lg:backdrop-blur-none">{children}</div>
      </div>
    </div>
  );
}