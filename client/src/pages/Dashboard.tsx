  ...
  return (
    <div className="flex h-dvh min-h-0 overflow-hidden bg-background">
      ...
      >
        ...
        {/* Account control: exactly ONE.

            The sidebar used to stack two things here — a static
            avatar/name/email row and, separately, a Sign out button. Both were
            passed to AccountSwitcher as `children`, which is the markup it
            falls back to when there is only one context, so that stack was what
            a single-context account actually saw.

            `alwaysShowTrigger` removes the fallback entirely: the switcher's own
            trigger is now the only control, it shows the account, and expanding
            it reveals the context options with Sign out already inside that
            menu. No identity row, no separate Sign out button, nothing stacked. */}
        <div className="border-t border-border px-3 py-3 shrink-0">
          <AccountSwitcher alwaysShowTrigger />
        </div>
      </aside>
